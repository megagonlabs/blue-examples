...
## v3: Plan Generation and Execution
...
- In the session, include the following agents:
    ...
    - Note: With the latest blue library/platform, `BLOCKING_OPENAI_AGENT` is automatically added to the session by the coordinator.
...
1. DEFAULT input (task decomposition & plan submission)
   ...
   - EOS:
     ...
     - Convert the parsed LLM plan into a Blue `Plan` with `compile_action_plan(worker, plan_dag, task)`:
       ...
       - Connect the sink node back to the planner by linking the sink agent's output to this planner agent's input `RESULT_EXECUTION`.
