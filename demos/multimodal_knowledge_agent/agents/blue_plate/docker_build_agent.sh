...
echo "${BLUE_CORE_DOCKER_ORG}/blue-agent-blue_plate${BLUE_BUILD_IMG_SUFFIX}:v${BLUE_DEPLOY_VERSION}"
...
docker buildx build \
  ...
  ${BLUE_BUILD_PUBLISH} \
  -t ${BLUE_DEV_DOCKER_ORG}/blue-agent-blue_plate${BLUE_BUILD_IMG_SUFFIX}:v${BLUE_DEPLOY_VERSION} \
  -f Dockerfile.agent .
