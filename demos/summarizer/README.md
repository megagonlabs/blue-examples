...
The `SUMMARIZER` agent is defined [here](https://github.com/rit-git/blue/blob/dev/lib/blue/agents/summarizer.py))
...
To try out the agent, first follow the [quickstart guide](https://github.com/rit-git/blue/blob/dev/QUICK-START.md) to deploy the `Summarizer Agent` (`SUMMARIZER`), `Task Coordinator Agent` (`COORDINATOR`), `Query Executor Agent` (`QUERY_EXECUTOR`),  and `NL-to-SQL Agent` (`NL2SQL`) agents.
