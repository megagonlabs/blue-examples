...
The `DOCUMENTER` agent is defined [here](https://github.com/rit-git/blue/blob/dev/lib/blue/agents/documenter.py))
...
To try out the agent, first follow the [quickstart guide](https://github.com/rit-git/blue/blob/dev/QUICK-START.md) to deploy the `Document Builder Agent` (`DOCUMENTER`), `Task Coordinator Agent` (`COORDINATOR`), `Query Executor Agent` (`QUERY_EXECUTOR`),  `NL-to-SQL Agent` (`NL2SQL`), and `OPENAI Agent` (`OPENAI`) agents.
